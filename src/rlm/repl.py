...
class REPLExecutor:
    ...
    def _build_globals(self, env: Dict[str, Any]) -> Dict[str, Any]:
        ...
        restricted_globals['_inplacevar_'] = _build_inplacevar_guard()
...
def _build_inplacevar_guard():
    """Return a guard function that RestrictedPython uses for augmented assignments."""
    inplace_map = {
        '+=': operator.iadd,
        '-=': operator.isub,
        '*=': operator.imul,
    ...
    def _inplacevar(func_or_symbol, *args):
        """Dispatch to the correct in-place operator, supporting RestrictedPython's calling convention."""
        if callable(func_or_symbol):
            operator_fn = func_or_symbol
        else:
            operator_fn = inplace_map.get(func_or_symbol)
            if operator_fn is None:
                raise TypeError(f"Unsupported augmented assignment operator: {func_or_symbol}")
        return operator_fn(*args)
